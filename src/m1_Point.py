...
class Point(object):
    ...
    def move_to(self, newx, newy):
        d = math.sqrt((self.x-newx)**2+(self.y-newy)**2)
        self.x = newx
        self.y = newy
    ...
    def move_by(self, dx, dy):
        d = math.sqrt((self.x-(self.x+dx))**2+(self.y-(self.y+dy))**2)
        self.y = self.y + dy
        self.x = self.x + dx
        self.moves_made = self.moves_made + 1
        self.distance = self.distance + d
    ...
    def closer_to(self, p1, p2):
        d1 = math.sqrt((self.x-p1.x)**2+(self.y+p1.y)**2)
